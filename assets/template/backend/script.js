$(document).ready(function () {
    ...
    document.onmousemove = function(){ 
        ...
        }, 300000); //300000ms 5 minutos, 60,000 1 minuto
    }

    $(document).on('click', '#barcode_btn', function(e){
        e.preventDefault();
        var val1 = Math.floor(1000 + Math.random() * 99999);
        var val2 = Math.floor(1000 + Math.random() * 99999);
        $('[name="codigo_barras"]').val('7'+val1+val2+'1');
    })
    $(document).on("click",".btn-view-barcode", function(){
        ...
        for (var i = 1; i <= Number(cantidad); i++) {
            ...
            html += '<div class="div-precio">Q.'+precio+'</div>';
                ...
                success:function(data){
                    ...
                    if (data =="0") {
                        swal({
                            position: 'center',
                            type: 'warning',
                            title: 'El codigo de barras no esta registrado o no cuenta con stock disponible',
    ...
    $(document).on("keyup mouseup","#tbventas input.cantidadesVenta", function(){
        ...
        if (cantidad!='') {
            if (cantidad == 0) {
                alertify.error("El valor ingresado no puede ser menor a la unidad");
                $(this).val('1');
                importe = precio;
            }else if(cantidad > stock){
                alertify.error("El valor ingresado no puede sobrepasar el stock");
                $(this).val(stock);
    ...
    /*
    Para imprimir directamente el ticket
     $("#form-venta").submit(function(e){
        $('button[type=submit]').attr('disabled','disabled');
        $('button[type=submit]').text('Procesando...');
        e.preventDefault();

        cantidadproductos = $("#tbventas tbody tr").length;
...
*/
    ...
    $("#year").on("change",function(){
     ...
     $(document).on("click", ".btn-check", function(e){
        ...
        var ruta = $(this).attr("href");
        swal({
                title:"Esta seguro que desea activar este registro?",
                text: "Activar",
                type: "warning",
                showCancelButton: true,
                confirmButtonClass: 'btn btn-success',
    ...
    //Mostrar informacion de los Modales
